...
diverse posizioni
con centri $O_{\scriptscriptstyle A}$, $O_{\scriptscriptstyle B}$ e $O_{\scriptscriptstyle C}$.
...
${x_i}^*\left(\tau\right)$ e con ${y_i}^*\left(\tau\right)$
...
\begin{multline}
{x_i}^*\left(\tau_{\scriptscriptstyle B}\right)=
...
otteniamo infine
\begin{equation}
{x_i}^*\left(\tau_{\scriptscriptstyle B}\right)=\left(r_1+r_r\right)\cos\left(t_{\scriptscriptstyle B}\right) -
...
di ${y_i}^*\left(\tau_{\scriptscriptstyle B}\right)$ che coincide naturalmente
con la
...
otteniamo
\begin{multline}
\dfrac{{\rm d}{x_i}\left(\tau\right)}{{\rm d}\tau}\bigr|^*_{\scriptscriptstyle B} = 
...
\begin{equation}
\dfrac{{\rm d}{y_i}\left(\tau\right)}{{\rm d}\tau}\bigr|*_{\scriptscriptstyle B} = 
...
\dfrac{\dfrac{{\rm d}{x_i}\left(\tau\right)}{{\rm d}\tau}\bigr|^*_{\scriptscriptstyle B}}
{\dfrac{{\rm d}{y_i}\left(\tau\right)}{{\rm d}\tau}\bigr|^*_{\scriptscriptstyle B}} \,,
\label{eq:dtot}
\end{equation}
...
che garantiscano la trasmissione omocinetica del moto, 
aventi primitive circolari. Contrariamente 
...
in due porzioni distinte: considerando il punto di contatto tra le
primitive, una di queste porzioni si trover\`a 
dalla parte della prima ruota mentre l'altra porzione si trover\`a
dalla parte della seconda ruota. Contenendo, poi, ciascuna delle porzioni
di profilo ora citate, due tratti distinti, un primo di epicicloide
e un secondo di ipocicloide,
il numero di tratti da considerare sar\`a in definitiva quattro.
Ad esempio, volendo
...
dei quali si adattano forzatamente ad essere legati al modulo della ruota,
...
loro numero dei denti al loro diametro primitivo, ma soprattutto individua
...
legami tra le quantit\`a geometriche che modellano, in quell'ambito,
...
tali punti con delle rotelle di raggio  opportuno
...
\`E facile comprendere che i fianchi rettilinei dei denti
...
In un ambito dove la minuscola quantit\`a di energia disponibile proviene da
