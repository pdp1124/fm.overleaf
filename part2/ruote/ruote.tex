\chapter{Trasmissione del Moto Tramite Ruote Dentate}\index{ruote!dentate}\label{cap_ruote_ev}
...
ruote, che si manifesta tramite una serie di sporgenze opportune, chiamate denti, e 
immaginiamo di costruire due cilindri di diametri adeguati ai nostri
scopi e di calettarli sui
...
a denti diritti.
Assicuriamo coloro che volessero
...
questo argomento \`e esposto in modo rigoroso e completo in
...
che chiameremo {\em circonferenze primitive}\index{circonferenze primitive},
...
di garantire il corretto rapporto di trasmissione,
...
profili coniugati lisci e privi di singolari\`a sarebbe superiore.
...
tale da portare a coniugazione i punti delle due curve aventi la stessa
...
importanza nella fabbricazione di famiglie di ruote dentate che possano
 ingranare l'una con l'altra.
...
e diventano le {\em primitive del moto}\index{primitive del moto}.
\begin{figure}[hbt]
\begin{center}
...
le ruote dentate con denti profilati ad evolvente di cerchio,
...
profili ad evolvente tramite circonferenze primitive diverse da quelle di
partenza la
...
Il fatto che i profili ad evolvente siano, entro limiti ragionevoli,
...
$PA$) si determina applicando il teorema del coseno al triangolo $\triangle
{O_1BP}$ 
\begin{equation}
(PB)^2  +2r_p\sin( \theta)(PB)+r_p^2 - r_t^2=0\,,
\label{eq:segpb2}
\end{equation}
\noindent pertanto
\begin{equation}
(PB)= -r_p\sin( \theta)\pm \sqrt{r_p^2\sin^2(\theta)-r_p^2 + r_t^2}\,.
\label{eq:segpb3}
\end{equation}
\noindent Introducendo le grandezze legate al proporzionamento normale 
e facendo il rapporto tra $(PB)$ e il passo base $p_b$ abbiamo
\begin{equation}
\frac{(PB)}{p_b}= \frac{-(z/2)\sin( \theta)\pm \sqrt{(z/2)^2\sin^2(\theta)+ z+1}}{\pi \cos (\theta)}\,.
\label{eq:segpb3_1}
\end{equation}
\noindent La formula \ref{eq:segpb3_1} fornisce,
con  $z=17$, un fattore di ricoprimento,
nel tratto di competenza $PB$, $\underline f_c=0.75$. La qual cosa  significa
...
\section{Ruote Corrette Tramite Spostamento del Profilo I} \label{ruote-corr1}
...
\section{Ruote Corrette Tramite Spostamento del Profilo II}
...
dentate, sempre con $z_1=11$ e $z_2=60$ denti. Come si legge nella didascalia,
...
ci assicurano che i nostri profili ad evolvente sono ancora coniugati.
...
$\cos(\theta_f)$, perci\`i anche l'interasse di funzionamento $i_f$, dipende
...
con denti profilati ad evolvente che dominano
...
Quindi profilo ad evolvente ovunque, sulle ruote
...
\ref{fig:dentiera_maag},
 ci fornisce automaticamente le ruote dentate ad evolvente,
...
sospettiamo per\`o che il profilo ad evolvente
